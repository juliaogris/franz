...
run: install  ## Run franz golang kafka client
	franz
...
produce:  ## Produce 3 events (EVENT=ping make produce)
	kafkactl produce heartbeat --key "heart" --value "$(EVENT) 1"
	kafkactl produce heartbeat --key "heart" --value "$(EVENT) 2"
	kafkactl produce heartbeat --key "heart" --value "$(EVENT) 3"

consume:
	kafkactl consume heartbeat -b
